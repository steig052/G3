...
const int ForceInputPins[2]    = {0, A7};
...
int Flex_baseline[3] = {1023, 1023, 1023}, Force_baseline[2] = {1023, 1023};
int FlexMax[3] = {0, 0, 0}, ForceMax[2] = {0, 0}; 
int flexState[3] = {0, 0, 0};
int prevflexState[3] = {0, 0, 0};
...
if (num == 3) /* index finger */ {
  if (FlexReading <= Flex_baseline[num] +  0.8*(FlexMax[num] - Flex_baseline[num])){
    state[num] = 1;
  } else {state[num] = 0;}
}
else{ /* other fingers */
...
if(ble.isConnected()){
  ...
  sendKeycode(14); /* Signaling Flex Sensor baseline is logged. UnityScene: Request User to curl fingers */
  ...
  delay(10000); /* UnityScene: Thumb Force Sensor */
  ...
  sendKeycode(14); /* Signaling baseline is logged. UnityScene: Request User to pinch */
      /* Start timer on glove and Unity if sensing change */
      bool changed2 = false; 
      while(!changed2){
  ...
  delay(10000); /* UnityScene: Palm Force Sensor */
  Force_baseline[1] = 1023-analogRead(ForceInputPins[0]);
